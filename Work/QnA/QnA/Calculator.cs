...
namespace QnA
{
    class Calculator
    {
        ...
        public List<MainStaff.VariableData> getQuestionWithRandomNumbers(string q)
        {
            ...
            {                
                ...
                foreach (var item in str)
                {
                    if (item.Contains('='))
                    {
                        //만약 아무것도 안넣었을 경우 예: x . range를 얼마를 둬야하나.
        ...
        {   
            ...
            //equation = bracketCalculator(equation);
        ...
        private string ConverterTrig(string x){
            string eq = x;
            string[] trig = new string[] { "sin", "cos", "tan" };
            if (eq.Contains("cos") || eq.Contains("sin") || eq.Contains("tan"))
            {
